...
\     */
    ...
    private function configureView($view)
    {
        ...
        $view->nonce = Nonce::getNonce('LoginRevokable.login');
    }
    ...
    {
        Nonce::discardNonce('LoginRevokable.login');
	...
	Db::get()->query("DELETE FROM " . Common::prefixTable('revokable_auth') . " WHERE auth_user = ?", array($this->auth->getLogin()));
        if (empty($logoutUrl)) {
